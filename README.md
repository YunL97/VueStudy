...
* 뷰2 의 한계점
  * 슬롯
  * 믹스인: 비지니스 로직에 밀접하게 연관된 로직들을 재사용하기위해 보통 믹스인을 사용 -> 믹스인은 특정컴포넌트데 두개이상 사용하는 순간 로직의 흐름을 따라가기가 어렵다 -> 엄격한 규칙으로 사용범위와 변수명을 제한해야한다
  * 하이오더 컴포넌트
* 뷰 3에서 달라진점
  * Teleport
  * 템플릿 표현식 관련 추가문법제공: 내장된 표현식을 허용하는 문자열 리터럴로 / 문자열삽입, 여러줄 문자열, 문자열 형식화, 문자열 태깅등 다양한 기능을 제공
  * Suspense
  * Reactivity 주입 API
  * Composition API
* 디렉티브 문법부터 공부 